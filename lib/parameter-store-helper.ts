...
export class ParameterStoreHelper {
  ...
  /**
   * Create parameters for Lambda function ARNs
   * @param functions - Object containing Lambda functions
   * @param environment - Environment name
   */
  public createLambdaFunctionParameters(
    ...
    Object.entries(functions).forEach(([name, func]) => {
      this.createParameter(
        `${name}FunctionArn`,
        func.functionArn,
        `ARN of the ${name} Lambda function`,
        `/acorn-pups/${environment}/lambda-functions/${name}/arn`
      );

      this.createParameter(
        `${name}FunctionName`,
        func.functionName,
        `Name of the ${name} Lambda function`,
        `/acorn-pups/${environment}/lambda-functions/${name}/name`
      );
    });
  }
} 